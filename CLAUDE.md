...
- **Correction Critique du Service Worker & Élimination de `no-response` (`frontend-next/src/app/sw.ts`)** :
  * Ajout de `serwist.setCatchHandler()` pour capturer proprement les échecs de stratégie hors-ligne (ex: requêtes RSC Next.js `_rsc=...` et navigations HTML).
  * Les exceptions console `Uncaught (in promise) no-response` et les erreurs `ERR_FAILED` sont entièrement éliminées. Le SW renvoie désormais un payload RSC valide ou la page de secours HTML 200 OK.
...
- **Traçabilité & Journal d'Audit (`boutique_logs` & `BoutiqueLogs.tsx`)** :
  * Enregistrement automatique dans le **Journal d'Audit & Sécurité** de toutes les ouvertures de session, clôtures Z et consultations/impressions du Rapport X avec le nom du caissier, le total du CA et l'adresse IP.
  * Ajout du filtre dédié **`Sessions Caisse & Rapport X`** dans l'interface du Journal d'Audit marchand.
...
- **Refonte Ergonomique Barre de Recherche & Scanners (Responsive)** : Correction du chevauchement inesthétique des boutons "Scanner Caméra" et "Douchette Smartphone" sur mobile.
- **Déplacement du Bouton Vue Catalogue** : Le bouton de basculement d'affichage (Liste / Mosaïque) a été déplacé sous la barre de recherche, à gauche de la liste des catégories, pour libérer de l'espace en haut et harmoniser l'interface.
...
### [2026-08-09] - Refonte du Hero "Bento Box" et Carrousel des Fonctionnalités
- **Refonte Structurelle (Bento Box) (`frontend-next/src/app/boutiques/page.tsx`)** : Refonte asymétrique complète du layout de l'en-tête (Hero) pour l'annuaire des boutiques. La zone de recherche et les filtres sont désormais pleinement intégrés à la colonne de gauche (qui s'étire dynamiquement pour éviter tout vide central), tandis que la colonne de droite accueille les nouvelles fonctionnalités.
- **Carrousel Interactif (`frontend-next/src/app/boutiques/HeroCarousel.tsx`)** : Remplacement de l'ancienne carte statique "Boutique Taf Taf" par un composant Carrousel moderne, défilant automatiquement. Il met en évidence 4 fonctionnalités clés (Boutique Taf Taf, Caisse POS, Zéro Commission WhatsApp, Vendeurs Vérifiés) avec pause au survol et navigation manuelle.
- **Enrichissement Typographique et Alignement** : Ajout de puces de réassurance ("0% de commission", "100% Vendeurs vérifiés", "Contact direct") dans le texte de présentation pour combler harmonieusement l'espace vertical. Utilisation de `align-items: stretch` et `margin-top: auto` pour garantir un alignement parfait de la barre de recherche avec le bas des widgets de la colonne de droite.

---### [2026-08-08] - Amélioration du formulaire de création de boutique et UX de recherche
...
- **UX de Recherche (`frontend-next/src/app/page.tsx`, `frontend-next/src/app/SearchBar.tsx`)** : Ajout d'une ancre `#resultats` sur le conteneur principal de la page d'accueil. Conversion de la barre de recherche en composant client (`next/navigation`) et modification de tous les liens (catégories, budget, tri, tendances) pour qu'ils déclenchent un défilement automatique immédiat vers les résultats lors du clic, améliorant considérablement l'expérience utilisateur sur mobile et desktop.
- **Menu Boutique (`frontend-next/src/app/boutique/BoutiqueClient.tsx`)** : Transformation du menu de navigation (sidebar) en accordéon. Les groupes ("Finance & Rapports", "Paramètres & Équipe", etc.) sont désormais repliables pour optimiser l'occupation verticale sur mobile. Par défaut, seul le premier groupe ou celui contenant l'onglet actif est ouvert.
- **Affichage Catégories Mobile (`frontend-next/src/app/page.tsx`, `globals.css`)** : Remplacement de l'affichage en grille désordonnée des catégories sur la page d'accueil par un défilement horizontal fluide sur mobile (comme sur Instagram/Airbnb). Cela aligne parfaitement toutes les pilules sur une seule ligne glissante.
- **Refonte Dashboard Boutiques (`frontend-next/src/app/boutique/BoutiqueClient.tsx`)** : Élargissement du conteneur de liste des boutiques à 1200px et passage en affichage grille multi-colonnes. La carte `BoutiqueCard` a été repensée : boutons secondaires (Modifier, Voir, Supprimer) transformés en icônes discrètes en haut à droite, informations de contact alignées avec des icônes Lucide, et bas de carte réservé aux larges boutons d'actions principales ("Gérer", "Caisse POS").
- **Refonte Annuaire Boutiques (`frontend-next/src/app/boutiques/page.tsx`, `BoutiquesSearch.tsx`)** : Intégration de la barre de recherche directement à l'intérieur du bloc Héro (bannière) pour combler le vide central sur grands écrans. Refonte de la barre de recherche (style Airbnb : plus large, grande police, bords très arrondis, ombre portée élégante) et centrage de la liste des catégories sous la bannière.
...
- **Intégration du Portail Développeur API dans l'Espace Marchand (`frontend-next/src/app/boutique/PortailDeveloppeurBoutique.tsx` & `BoutiqueClient.tsx`)** :
  - Création du composant marchand `PortailDeveloppeurBoutique.tsx` et ajout de l'onglet **`🔌 Portail Développeur API`** dans le menu latéral de gestion de boutique sous *"Paramètres & Équipe"* (accessible exclusivement sur le plan Business VIP).
  - Correction de la correspondance du champ serveur `api_key` : la clé complète s'affiche désormais instantanément dans un encadré vert avec un bouton **`📋 Copier`** et est automatiquement copiée dans le presse-papier lors du clic sur *"Générer une Clé API"*.
  - Sécurisation hermétique des 6 endpoints backend (`GET/POST/DELETE /api/boutiques/:id/api-keys` et `GET/POST/DELETE /api/boutiques/:id/webhooks`) avec le middleware `requireBusiness`.
...
- **Remplacement des Émojis par les Véritables Icônes Vectorielles SVG Réseaux Sociaux (`layout.tsx`, `globals.css`)** :
    - Remplacement complet des émojis génériques (🎵, 📢, f, 📸, 𝕏) du pied de page par les véritables logos vectoriels SVG officiels et haute définition de **TikTok**, **WhatsApp (Canal)**, **Facebook**, **Instagram** et **X / Twitter**.
    - Ajout de boutons circulaires surélevés (`.footer-social-link`) avec animations fluides au survol (`transform: translateY(-3px)`) et couleurs officielles de chaque réseau (Vert WhatsApp `#25D366`, Bleu Facebook `#1877F2`, Dégradé Instagram, Noir/Cyan TikTok `#00f2fe`).
- **Audit Global des Fenêtres & Polices Système Unifiées (`frontend-next/src/app/globals.css`, Modales & Wizards)** :
  - **Règle Globale de Police Système (`globals.css`)** : Définition de la règle universelle `html, body, button, input, select, textarea { font-family: var(--font-inter), system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif !important; }`. Élimination définitive des rendus de polices génériques ou non stylisées sur 100% des pages et fenêtres.
  - **Harmonisation de la Couleur Bleu Marine Nopalou du Pied de Page (`.site-footer`)** : Correction et fixation explicite de la couleur de fond du pied de page sur le bleu marine Nopalou officiel (`#1C2B4A !important`), garantissant une parfaite identité visuelle avec le logo et la charte graphique Nopalou.
  - **Harmonisation des Modales & Fenêtres Interactives** :
    - *Wizard Forfait Télécom* (`WizardForfait.tsx`) : Application de la pile système native et refonte des boutons/puces.
    - *Fenêtre de Commande / Express Checkout* (`CommanderModal.tsx`) : Application de la police système native et harmonisation des onglets de canaux (WhatsApp / Formulaire).
    - *Importation par Lot de Catalogue* (`BatchImportModal.tsx`) : Remplacement de `font-archivo` par la pile système haute lisibilité sur la modale d'intake.
    - *Fenêtre de Déclaration / Paiement Manuel* (`ModalPaiementManuel.tsx`) : Application de la police système native sur le conteneur principal.
- **Enrichissement Harmonisé du Forfait Business VIP & Isolation Backend (`frontend-next` & `backend`)** :
  - Mise à jour complète de l'ensemble des pages et composants (`fonctionnalites-data.ts`, `TarifsPublicsSelector.tsx`, `ShowcaseTabs.tsx`, `/compte/fonctionnalites`, `/tarifs-boutique`, `/creer-boutique`) pour refléter les 8 piliers majeurs du forfait Business VIP (35 000 FCFA/mois) : Multi-Caissiers PIN & clôtures Z, Multi-Magasins & Transferts inter-boutiques, Portail Développeur API REST & Webhooks, Comptabilité avec marges nettes, Automation WhatsApp Paniers Abandonnés, Bannière sponsorisée prioritaire, Analytics CA & Classement vendeurs, Support VIP 7j/7.
  - Sécurisation backend sans aucune faille : injection du middleware `requireBusiness` / `checkAbonnement` sur les routes d'API Keys (`POST /:id/api-keys`), Webhooks (`POST /:id/webhooks`), Caissiers PIN (`POST /:id/caissiers`), et calcul dynamique du quota d'annonces classées (2 pour Taf Taf, 5 pour Pro, 15 pour Business VIP).
- **Contrôle Backend Anti-Contournement & Quotas Boutiques (`backend/routes/boutiques.js`)** :
  - Renforcement du contrôle anti-contournement par numéro de téléphone et e-mail. Normalisation universelle sur les 9 derniers chiffres du téléphone (`RIGHT(REGEXP_REPLACE(..., '[^0-9]', '', 'g'), 9)`) et comparaison insensible à la casse des e-mails (`LOWER(u.email)`).
  - Élimination des contournements liés aux formats de saisie (`+221`, `221`, espaces, parenthèses) pour bloquer hermétiquement la création de plus de $N$ boutiques (défini dans l'admin `/admin/tarifs`).
...
- **Intégration de la Fiscalité, des Documents clients, des Fournisseurs et du mode Hors-ligne (POS)** (ajouté 4 août 2026) :
  ...
  - **Audit & Am�lioration de la Cat�gorisation** :
    - Enrichissement des mots-cl�s pour auto-moto, mode, immo et emploi (ajout de 'diesel', 'essence', 'shoes', 'wax', 'cherche boulot', etc.).
    - R�ordonnancement de l'�valuation des cat�gories dans scraper-immo-facebook.js (priorit� donn�e � auto-moto et immo pour �viter les faux positifs dans tv-electro ou informatique li�s aux mots comme '�cran' ou 'ordinateur').
    - Cr�ation et ex�cution d'un script de migration (ackend/scripts/reclassify.js) qui a r��valu� l'int�gralit� des annonces de la base de donn�es avec la nouvelle logique, corrigeant plusieurs centaines d'erreurs historiques.

  - **Scraper Facebook** : Ajout de 5 nouveaux groupes d�di�s � l'emploi pour augmenter le volume d'annonces de cette cat�gorie.

    - Ajustement du scraper : Les groupes d'emploi sont tous trait�s dans le tout premier lot de requ�tes (maxGroupes = 10) pour maximiser le remplissage imm�diat de la cat�gorie Emploi.

 # # #   R � g l e   U I   :   B a r r e s   d e   R e c h e r c h e 
 -   * * D � f i l e m e n t   a u t o m a t i q u e   ( A n c r e   # r e s u l t a t s ) * *   :   T o u t e s   l e s   b a r r e s   d e   r e c h e r c h e   s i t u � e s   s u r   d e s   p a g e s   c o n t e n a n t   u n   e n - t � t e   ( H e r o )   d o i v e n t   a j o u t e r   l ' a n c r e   # r e s u l t a t s   l o r s   d e   l a   s o u m i s s i o n   (  o u t e r . p u s h ( ' / p a g e ? q = . . . # r e s u l t a t s ' ) )   p o u r   q u e   l ' u t i l i s a t e u r   a t t e r r i s s e   d i r e c t e m e n t   s u r   l e s   r � s u l t a t s ,   e n   s a u t a n t   l e   H e r o . 
  
 
    - Affichage Annonces : Modification de l'API (routes \nnonces.js\ et \search.js\) pour syst�matiquement prioriser les annonces natives (cr��es directement sur Nopalou) au-dessus des annonces import�es depuis Facebook dans l'ordre d'affichage par d�faut.

- Refonte UI Guides : Stylisation premium des composants guide (Achat, Immo, Forfait) avec correction des espacements, refonte du bouton de retour et am�lioration globale de l'interface (globals.css, composants React).
