    ...
    - **PDF aux standards OHADA** : Refonte complète de la génération PDF (`GET /api/boutiques/:id/documents/:docId/pdf`) :
      ...
      - **Audit & Correction de Cohérence Photo-Produit du Catalogue Standard (`generate-catalog.js` & `catalogues-standards.json`)** :
        ...
        - **Refonte Globale & Épurée de la Page d'Accueil (`frontend-next/src/app/page.tsx`)** :
          ...
          - **Aplatissement et Densification Extrême des Filtres (Zéro Vide Vertical)** : L'ancienne architecture en colonnes (`flex-direction: column`) causait d'énormes vides verticaux lorsque les filtres passaient à la ligne. Le layout a été totalement "aplati" en une seule rangée `flex-row` fluide. Les filtres s'alignent désormais horizontalement sur une seule ligne (Budget, État, Trier, Tendances) pour occuper parfaitement la largeur sans jamais gaspiller de hauteur. Le CTA "⚡ Ouvrir une Boutique Pro" (Premium sombre) est poussé à l'extrême droite.
