...
- **Intégration du Portail Développeur API dans l'Espace Marchand (`frontend-next/src/app/boutique/PortailDeveloppeurBoutique.tsx` & `BoutiqueClient.tsx`)** :
  ...
  - Sécurisation hermétique des 6 endpoints backend (`GET/POST/DELETE /api/boutiques/:id/api-keys` et `GET/POST/DELETE /api/boutiques/:id/webhooks`) avec le middleware `requireBusiness` : tout appel provenant d'une boutique non-Business VIP (Taf Taf ou Pro) est automatiquement rejeté HTTP 403.
