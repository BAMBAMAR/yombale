- **Fiabilisation & Éradication des Photos Par Défaut dans la Baguette Magique (Import Rapide E-Commerce) (`main` - 23 août 2026)** 🌟🪄📸🛠️✨ :
  * **🪄 Extraction Réelle & Élimination Définitive des Photos/Titres Génériques Fixes (`backend/services/magic-import.js`)** :
    ...
    - **Filtrage Anti-Logos & Priorisation Visuels Produits (`BAD_IMAGE_PATTERNS`)** : Exclusion stricte des icônes de paiement et partenaires de livraison SHEIN (*InPost, Poste Italiane, PayPal, Klarna, Gift, Wallet, images3_ccc, images3_acp*) et priorisation des vraies photos produits du catalogue (*`_pi/`*).
    - **Rejet des Slogans Portails & Extraction Réelle SHEIN (`GENERIC_SLOGANS_REGEX`)** : Filtrage et rejet automatique des slogans marketing génériques de page d'accueil SHEIN (*« Vêtements homme & femme, shoppez la mode en ligne »*, *« Des chaussures aux vêtements... »*) au profit de l'extraction intégrale du vrai nom produit dans le slug URL (`1-5-10-15pcs-Transparent-Plastic-Storage-Bags...`).
    ...
    - **Alignement Catégoriel Nopalou & Rangement / Outillage** : Classification automatique du rangement et organisation voyage (`storage`, `organizer`, `shoe bag`, `packing bag`, `housse`) dans **🏠 Maison & Décoration**, et de l'électroportatif dans **🧱 Quincaillerie & BTP**.
  * **🛡️ Protection de l'État Formulaire, Persistance des Images & Alignement UX (`frontend-next/src/app/boutique/BoutiqueClient.tsx`)** :
    - **Alignement Horizontal Parfait des Champs de Prix** : Unification de la hauteur des libellés (`minHeight: 34px`, `alignItems: 'flex-end'`) pour aligner rigoureusement sur la même ligne horizontale les champs *Prix de vente*, *Prix d'achat / Coût* et *Prix barré / Promo*.
  ...
  * **🧪 Validation & Contrôle Qualité** :
    ...
    - Tests d'extraction réels sur liens SHEIN (`17722752` Sacs de rangement transparents voyage) : Titre exact, catégorie `maison`, 5 photos HD réelles extraites (0 logo InPost / Poste Italiane).
...
- **Audit & Correction Intégrale des Demandes de Crédit, Approbation & Carnet de Dettes (16 août 2026)** 💳 :
  ...
  * **Implémentation des Route Handlers Next.js Manquants (`frontend-next/src/app/api/boutiques/...`)** :
    1. Ajout de la méthode `DELETE` dans `credits-clients/[clientId]/route.ts` pour proxyfier la suppression de profil client sans erreur `405 Method Not Allowed`.
    2. Création de la route `PATCH` `credits-clients/[clientId]/statut/route.ts` pour le blacklisting et la réactivation des comptes clients.
    3. Création de la route `POST` `credits-clients/approuver-commande/route.ts` pour l'approbation d'achat à crédit et l'alimentation automatique du carnet.
  * **Modale de Notification Centrée sur la Page Web (`DrawerCart.tsx` & `CommanderModal.tsx`)** : Lorsqu'un client soumet une demande d'achat à crédit (ou une commande), une modale de confirmation élégante avec fond flouté (`backdropFilter: blur(4px)`) s'affiche désormais directement au centre de l'écran principal de la page web avec l'icône `💳`, l'explication explicite de l'envoi au Carnet du marchand, et le bouton d'action rapide WhatsApp Direct, évitant que la confirmation ne reste confinée dans le tiroir de panier.
  * **Réactivité Instantanée & Suppression du Rechargement F5 (`CarnetDettes.tsx`)** : Mise à jour réactive immédiate de l'état local React lors du clic sur `Approuver` ou `Rejeter`. L'encadré bleu de demande disparaît instantanément et la carte du nouveau client (avec son solde et ses coordonnées) s'insère automatiquement en haut de la liste des débiteurs à l'écran, sans aucun délai ni besoin de rafraîchir manuellement la page (F5).
  * **Séparation Stricte des Fiches Clients par Numéro de Téléphone (`backend/routes/boutiques.js`)** : Correction du ciblage client lors de l'approbation d'achat à crédit. La recherche de profil privilégie désormais à 100% le numéro de téléphone unique du client, empêchant deux personnes différentes portant le même prénom/nom (ex: "Amar") d'être fusionnées à tort sur la même fiche si leurs numéros de téléphone sont différents.
  * **Protection Anti-Blacklist au Panier Web (`backend/routes/comptabilite.js`)** : Ajout d'une vérification préalable dans `creerCommandeBoutique` pour refuser immédiatement le passage de commande à crédit si le client est enregistré au statut `bloque` par la boutique (`⛔ Votre compte est actuellement bloqué par la boutique pour les achats à crédit.`).
  * **Actions d'Approbation & Rejet de Demande (`CarnetDettes.tsx` & `Commandes.tsx`)** : Ajout des boutons d'action `❌ Rejeter` à côté de `✅ Approuver & Ajouter au Carnet` sur chaque demande de crédit en attente avec option de notification de refus explicite sur WhatsApp.
- **Module de Partage par QR Code & Lien Vitrine avec Validation Instantanée Marchand (16 août 2026)** 📱 :
  ...
  * **Passage de Pré-commande & Demande de Crédit Client (`BoutiqueDetailClient.tsx`, `DrawerCart.tsx` & `CommanderModal.tsx`)** : Les clients scannant le QR code en boutique ou accédant au lien peuvent composer un panier multi-produits avec détection automatique de la bannière `💳 Mode Demande d'Achat à Crédit`, affichage d'un écran de succès explicite `💳 Demande d'Achat à Crédit Transmise !` expliquant les étapes de validation au carnet, et enregistrement automatique en base des commandes déclenchées par le bouton WhatsApp Direct.
  * **Intégration Chatbot WhatsApp Meta (`backend/services/whatsapp-chatbot.js`)** : Ajout du bouton interactif `💳 Demande Achat Crédit` (ID `pay_credit`) dans l'arbre conversationnel du Chatbot WhatsApp Meta. Les clients commandant via le bot WhatsApp peuvent désormais sélectionner directement l'option crédit, qui transmet la demande au carnet du marchand.
  * **Approbation & Alimentation Automatique du Carnet (`backend/routes/boutiques.js`, `Commandes.tsx` & `CarnetDettes.tsx`)** : Point de terminaison API `/api/boutiques/:id/credits-clients/approuver-commande` révisé avec recherche flexible par téléphone/nom, réactualisation prioritaire synchrone (`chargerDonnees()`), émission de l'événement réactif `carnet_updated`, et boîte de dialogue optionnelle pour la confirmation WhatsApp sans bloquer l'affichage immédiat du client dans le Carnet.
  * **Gestion Avancée des Clients Carnet : Blacklist, Réactivation & Suppression (`backend/routes/boutiques.js` & `CarnetDettes.tsx`)** : 
    - Ajout de la colonne `statut` (`actif`, `bloque`, `archive`) sur la table `caisse_clients_credits`.
    - Nouveaux endpoints backend : `PATCH /api/boutiques/:id/credits-clients/:clientId/statut` et `DELETE /api/boutiques/:id/credits-clients/:clientId`.
    - Boutons d'action `⛔ Blacklister` et `🟢 Réactiver` sur chaque carte client dans le Carnet. Les clients blacklistés ne peuvent plus valider d'achat à crédit (`approuver-commande` rejette automatiquement les comptes bloqués avec un message explicite).
    - Bouton `🗑️ Supprimer` pour effacer définitivement le profil et l'historique d'un client.
  * **Découplage Annuaire Web, Authentification Souple & Sécurisation SQL (`backend/routes/boutiques.js`, `backend/routes/comptabilite.js` & `CarnetDettes.tsx`)** : Passage de `GET /api/comptabilite/:boutiqueId/commandes` en `tokenOptional` avec recherche universelle par UUID ou slug pour éliminer tout blocage `401 Unauthorized`. Sécurisation du ciblage par référence (ex: `C-MSW5MVBU`) ou UUID dans `approuver-commande` pour empêcher l'erreur `500 Internal Server Error`.
