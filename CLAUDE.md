...
- **Restauration Intégrale du Logo Officiel Orange Nopalou (PWA Android / iOS / Desktop / Splash Screen / Manifest) & Correction de l'Onglet Comptabilité (`icon-512.png`, `icon-192.png`, `icon-maskable-512.png`, `icon-512.svg`, `logo-mark.svg`, `apple-icon.tsx`, `manifest.json`, `layout.tsx`, `BoutiqueClient.tsx`, `CLAUDE.md`) (`main` - 26 août 2026)** 🟧✨📱💰🛡️ :
  * **🟧 Remplacement Définitif & Rendu Inset Squircle des Icônes PNG & SVG (`icon-512.png`, `icon-192.png`, `icon-512.svg`, `logo-mark.svg`)** :
    ...
    - Harmonisation complète sur PNGs (512, 192, 180 Apple) et SVGs vectoriels.
...
- **Refonte Architecturale E-Commerce Majeure — Variantes SKU Réelles, Décomposition des Commandes Multi-Produits avec Décrément de Stock, Catégories Parfumerie & Optique et Monogrammes Vectoriels (`backend/migrate-inline.js`, `comptabilite.js`, `boutiques.js`, `CartContext.tsx`, `DrawerCart.tsx`, `ProduitCTA.tsx`, `BoutiqueDetailClient.tsx`, `categories.ts`, `CLAUDE.md`) (`main` - 24 août 2026)** 🛒📦💎✨🌸 :
  ...
  * **🛍️ Panier & Tunnel d'Achat Multi-Variantes (`CartContext.tsx`, `DrawerCart.tsx`, `layout.tsx`)** :
    ...
    - **Nettoyage Paramètres & Fix Scroll / TDZ (`BoutiqueClient.tsx`)** : Suppression du sélecteur de statut de visibilité dupliqué dans les paramètres de la boutique, élimination du scroll intempestif vers le haut grâce au verrou `handledRef`, et correction de l'ordre d'initialisation de `router = useRouter()` (résolution du `ReferenceError: Cannot access 'router' before initialization`).
