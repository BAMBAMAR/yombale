    ...
    - **PDF aux standards OHADA** : Refonte complète de la génération PDF (`GET /api/boutiques/:id/documents/:docId/pdf`) :
      ...
      - **Audit & Correction de Cohérence Photo-Produit du Catalogue Standard (`generate-catalog.js` & `catalogues-standards.json`)** :
        ...
        - **Refonte Globale & Épurée de la Page d'Accueil (`frontend-next/src/app/page.tsx`)** :
          ...
          - **Allègement Visuel & Alignement Parfait de la Barre de Filtres** : Suppression du gros module promotionnel noir ("Développez vos ventes") à droite de la barre de filtres, qui alourdissait inutilement la navigation. Il a été remplacé par un espace pertinent aligné au pixel près avec les autres filtres. La colonne de droite contient désormais les "Top Marques" et des boutons d'actions utiles (ex: **"✖ Effacer les filtres"** dynamiquement affiché, et un accès rapide aux **"🛡️ Boutiques vérifiées"**). L'alignement vertical entre les 3 colonnes est désormais mathématiquement exact.
