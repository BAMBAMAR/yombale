    ...
    - **PDF aux standards OHADA** : Refonte complète de la génération PDF (`GET /api/boutiques/:id/documents/:docId/pdf`) :
      ...
      - **Audit & Correction de Cohérence Photo-Produit du Catalogue Standard (`generate-catalog.js` & `catalogues-standards.json`)** :
        ...
        - **Refonte Globale & Épurée de la Page d'Accueil (`frontend-next/src/app/page.tsx`)** :
          ...
          - **Ajustement UX du Hero (Proportions, Alignement & Pleine Largeur)** : Les catégories ont été sorties de la colonne centrale pour s'étaler sur la pleine largeur en dessous de la grille. Résultat : la hauteur du Hero est drastiquement réduite, et les cartes WhatsApp / Taf Taf sont parfaitement alignées à hauteur du titre H1. Ajout d'un appel à l'action sur la carte WhatsApp pour équilibrer le design.
          - **Exploitation de l'espace vide des Filtres** : Le grand espace blanc inexploité à droite des filtres ("Budget" et "Trier") a été réorganisé en un layout à deux colonnes. Il héberge désormais un encart promotionnel sombre très premium ("⚡ Développez vos ventes") invitant les utilisateurs à créer leur vitrine, boostant ainsi la conversion sans surcharger la navigation mobile.
