...
- **Correction des Boutons Forçant le Paiement Manuel (`AnnoncesClient.tsx`, `BoutiqueClient.tsx`)** :
  ...
  * **Solution** :
    1. **Boost Annonce (`AnnoncesClient.tsx`)** : Suppression du bouton dupliqué en dur. Fusion dans un bouton unique `🚀 Booster 7j` qui tente d'abord le paiement en ligne Wave direct via `initierWaveBoost`. Si Wave est indisponible ou renvoie `fallback_manuel: true`, l'interface bascule automatiquement et de manière fluide vers la modale de paiement manuel pré-remplie (`boost_${userId}_${annonceId}`).
    2. **Gestionnaire Boutiques (`BoutiqueClient.tsx`)** : Remplacement du bouton ambigu *"Payer abonnement"* sur la carte boutique par un lien direct *"📖 Abonnements"* redirigeant le marchand vers `/boutique/abonnement` (avec choix des formules Pro/Business, de la durée de 1 à 12 mois avec réductions et choix Wave/OM). Le bouton *"🌟 Mettre en avant"* bascule également vers la modale manuelle si Wave est indisponible.
    3. **Sponsorings Produit & Immo (`SponsoringProduitBtn.tsx`, `SponsoringImmoBtn.tsx`)** : En cas d'échec ou de fallback Wave, l'interface ouvre automatiquement la modale manuelle avec référence unique au lieu d'afficher une erreur texte brute.
    4. **Server Actions & API Backend (`actions/paiement.ts`, `paiement.js`, `abonnements.js`, `wave.js`, `settingsCache.js`)** : Assouplissement des requêtes SQL d'appartenance d'annonce dans `boost/initier` et `annonce/initier` (évite les fausses erreurs 404), détection structurée de `fallback_manuel`, renvoi de la référence de paiement, support de `clientToken` (JWT `localStorage` en secours de la cookie session) et support dynamique de `wave_api_key` depuis les paramètres en base de données.

## 🚀 Mises à jour du 14/08/2026 : Système Universel de Basculement Automatique vers le Paiement Manuel (`ManualFallbackCard.tsx`, `PaiementErreurPage`, `CommanderModal.tsx`, `checkout-express/page.tsx`, `PaiementClient.tsx`, `whatsapp-chatbot.js`, `paiement.js`, `comptabilite.js`, `abonnements.js`)
- **Implémentation du Basculement Automatique vers le Paiement Manuel (Zero-Lost-Conversion)** :
  * **Problème** : Lorsque l'API Wave Checkout échouait (maintenance opérateur, quota, erreur réseau 5xx) ou que le client annulait son paiement sur l'application, l'utilisateur se retrouvait bloqué avec un message d'erreur et la transaction était perdue.
  * **Solution Universelle déployée sur TOUS les modules** :
    1. **Carte d'Alternative Manuelle (`ManualFallbackCard.tsx` & `PaiementErreurPage`)** : Intégration sur la page d'erreur de paiement d'un composant interactif guidé (*"💡 Alternative : Régler par Dépôt Manuel Direct"*) affichant le numéro marchand (`77 720 20 86`), un bouton 1-Clic *"Copier le numéro"*, le rappel de la référence et un formulaire rapide de saisie d'ID de transaction / preuve.
    2. **Gestionnaire de Basculement Automatique sur le Web (`CommanderModal.tsx`, `checkout-express/page.tsx`, `PaiementClient.tsx`)** : Si le serveur renvoie `fallback_manuel: true` lors de l'initialisation de Wave, l'interface bascule automatiquement l'onglet sur le paiement manuel et affiche les instructions de dépôt sans interrompre la commande.
    3. **Gestionnaire de Fallback Chatbot WhatsApp (`whatsapp-chatbot.js`)** : Si l'initialisation Wave échoue lors d'une commande WhatsApp, le chatbot envoie automatiquement les instructions de dépôt manuel direct (`77 720 20 86`) avec le montant total formaté.
    4. **Adaptation des APIs Backend (`paiement.js`, `comptabilite.js`, `abonnements.js`)** : Capture des exceptions API Wave et renvoi gracieux de la réponse `{ fallback_manuel: true, numero_depot: '777202086', reference, montant }`.

## 🚀 Mises à jour du 14/08/2026 : Correction du Message d'Erreur lors d'un Paiement Non Abouti / Solde Insuffisant dans les Boutiques (`PaiementErreurPage`, `comptabilite.js`, `boutiques.js`, `paiement.js`, `abonnements.js`)
- **Fix du Message d'Erreur Inapproprié sur Échec de Paiement (`frontend-next/src/app/paiement/erreur/page.tsx`, `backend/routes/comptabilite.js`, `backend/routes/boutiques.js`, `backend/routes/paiement.js`, `backend/routes/abonnements.js`)** :
  * **Problème** : Lorsqu'un utilisateur tentait d'acheter un produit dans une boutique mais annulait ou n'avait pas les fonds suffisants sur Wave, la page d'erreur affichait un message inapproprié concernant la publication d'une annonce classifiée (*"Paiement d'annonce non abouti — Votre annonce n'a pas été activée"*).
...
- **Exclusion du Scraping / Imports Externes des Notifications WhatsApp (`backend/routes/annonces.js`, `backend/services/notifications.js`)** :
  ...
  * **Solution (Option 2)** : Ajout d'une vérification stricte sur la provenance de l'annonce lors de la modération admin (`PUT /api/annonces/admin/:id` et `notifierModerationImmo`). Seules les annonces effectivement déposées sur le site Nopalou par un utilisateur (`url_source` et `ref_externe` nuls, `source` de type `'site'` / `'utilisateur'` / `'manuel'` / `'depot_gratuit'`) déclenchent désormais l'envoi de notifications WhatsApp. Les annonces issues de scrapers externes sont totalement ignorées pour la notification WhatsApp.
...
- **Fix du Chargement sur Nouveau Navigateur / Écran Dédié Caissier (`backend/routes/boutiques.js`, `CaisseClient.tsx`)** :
  * **Problème** : Sur un autre navigateur ou une tablette vierge (sans cookie de session administrateur), la boutique et la liste des caissiers ne s'affichaient pas car :
    1. L'endpoint backend `/api/boutiques/caisse-terminal/:token` ne rattachait pas le champ `plan_actif` à l'objet `boutique`, ce qui faisait échouer la validation d'autorisation `estBoutiqueAutorisee` côté client.
    2. En cas de statut HTTP 403/erreur, `CaisseClient.tsx` ignorait les données de secours et tentait un `getBoutiquesMine()` (réservé aux propriétaires connectés), échouant silencieusement sur les appareils caissiers.
  * **Solution** :
    1. Backend `backend/routes/boutiques.js` : Recherche élargie par `caisse_token`, `id` UUID ou `slug`. Rattachement automatique du champ `plan_actif` à l'objet boutique renvoyé et création systématique des caissiers par défaut si aucun caissier n'existe.
    2. Client `CaisseClient.tsx` : Lecture inconditionnelle de l'objet `boutique` et de ses caissiers/produits lors du démarrage par `initialToken`, avec mise en cache locale `localStorage` pour un fonctionnement ultra-rapide et hors-ligne.
...
 -   F o r � a g e   d e   w a v e A c t i f   =   t r u e   s u r   l e   f r o n t e n d   d a n s   l e s   c o m p o s a n t s   d e   p a i e m e n t   p o u r   s ' a s s u r e r   q u e   l e   b o u t o n   s ' a f f i c h e   t o u j o u r s   c o m m e   c ' � t a i t   l e   c a s   a u p a r a v a n t   s u r   l e   c o m p o s a n t   B o o s t .  
 
## ? Mises � jour du 14/08/2026 : Remplacement des Liens de Navigation Obsol�tes par le Composant PageHeader
- **Standardisation de la Navigation (Breadcrumbs & Retours)** :
  * **Probl�me** : Les pages affichaient des liens de navigation disparates avec le mot "Retour" ("? Retour aux annonces", "? Retour � la boutique", "? Retour � l'immobilier") cod�s en dur. Le design manquait d'uniformit�.
  * **Solution** : Syst�matisation de l'utilisation du composant PageHeader qui offre un fil d'Ariane clair, dynamique et stylis� pour l'ensemble du site.
    1. **Annonce Classique (nnonces/[id]/page.tsx)** : Remplacement du breadcrumb obsol�te et suppression du lien doublon de retour dans la barre lat�rale par un PageHeader complet.
    2. **Annonce Immobili�re (immo/[id]/page.tsx)** : Remplacement du fil d'Ariane texte et suppression des titres de page dupliqu�s par l'int�gration d'un PageHeader.
    3. **Boutique (outiques/[id]/page.tsx)** : Int�gration du PageHeader et suppression du lien textuel en bas de page.
    4. **Produit de Boutique (outiques/[id]/produits/[produitId]/page.tsx)** : Substitution du breadcrumb texte et �limination du lien "Retour � la boutique" par un PageHeader int�gr�.
    5. **Comparaison Immobili�re (immo/comparaison/page.tsx)** : Ajout du PageHeader pour clarifier la position dans l'arborescence, suppression des liens de Retour dans la page et sur les �tats d'erreur.

## ? Mises � jour du 14/08/2026 : Refonte et Modernisation des Pages de Paiement (Zero-Lost-Conversion)
- **Am�lioration du Design des Cartes de Paiement (Boost & Sponsoring)** :
  * **Probl�me** : La page de paiement pour le Boost 7j, ainsi que celles des Sponsorings Boutique/Produit/Immo, pr�sentaient un design basique (bordures dures, fond blanc plat, boutons g�n�riques) qui manquait d'un aspect premium et d'homog�n�it� avec le reste de la plateforme (Boutiques, Guides).
  * **Solution** : Refonte totale du composant de la carte de paiement (.paiement-card) dans globals.css et nettoyage des styles en dur dans les composants clients.
    1. **Design Premium (Glassmorphism & Ombres)** : Ajout de bordures arrondies fluides (24px), d'ombres port�es douces (ox-shadow) et d'animations de survol (�l�vation de la carte).
    2. **Typographie & Couleurs** : Utilisation d'un fond en d�grad� subtil pour l'en-t�te (#F8FAFC vers #F1F5F9) et d'un d�grad� de texte vert �meraude �l�gant pour afficher le montant total.
    3. **Boutons de Paiement Modernis�s** : Suppression des styles inline obsol�tes dans tous les fichiers Paiement*Client.tsx. Les boutons b�n�ficient d�sormais d'une esth�tique standardis�e (arrondis 16px, ombres douces sp�cifiques � Wave/OM, effets de sursis interactifs fluides et animation de fl�che).
  * Les 5 modules de paiement (payer-annonce, payer-boost, payer-sponsoring-boutique, payer-sponsoring-produit, payer-sponsoring-immo) sont d�sormais visuellement luxueux et rassurants.
