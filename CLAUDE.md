- **Module de Partage par QR Code & Lien Vitrine avec Validation Instantanée Marchand (16 août 2026)** 📱 :
  ...
  * **Passage de Pré-commande & Demande de Crédit Client (`BoutiqueDetailClient.tsx`, `DrawerCart.tsx` & `CommanderModal.tsx`)** : Les clients scannant le QR code ou accédant au lien peuvent composer un panier multi-produits et choisir le mode de paiement `💳 Demande d'Achat à Crédit (Carnet client)` avec notification immédiate au marchand.
  * **Approbation & Alimentation Automatique du Carnet (`backend/routes/boutiques.js`, `Commandes.tsx` & `CarnetDettes.tsx`)** : Nouveau point de terminaison API `/api/boutiques/:id/credits-clients/approuver-commande` et bouton `📒 Approuver & Ajouter au Carnet` qui crée ou retrouve le profil client, inscrit la vente à crédit au carnet client, met à jour le solde et déclenche une confirmation automatique par WhatsApp.
