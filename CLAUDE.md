...
- **Correction des Boutons Forçant le Paiement Manuel (`AnnoncesClient.tsx`, `BoutiqueClient.tsx`)** :
  ...
  * **Solution** :
    ...
    4. **Server Actions & API Backend (`actions/paiement.ts`, `paiement.js`, `abonnements.js`, `wave.js`, `settingsCache.js`)** : Détection structurée de `fallback_manuel`, renvoi de la référence de paiement, support de `clientToken` (JWT `localStorage` en secours de la cookie session) et support dynamique de `wave_api_key` depuis les paramètres en base de données.
