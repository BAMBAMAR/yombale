- **Module de Partage par QR Code & Lien Vitrine avec Validation Instantanée Marchand (16 août 2026)** 📱 :
  * **Nouveau Composant `QrCodeShareModal.tsx`** : Générateur de QR Code SVG imprimable et téléchargeable pour chaque marchand, avec copie rapide du lien direct de vitrine et partage sur WhatsApp.
  * **Intégration Tableau de Bord Marchand (`BoutiqueClient.tsx`)** : Bouton d'action rapide "📱 Mon QR Code & Lien" dans le header et le dashboard marchand pour afficher et partager son QR Code/Lien.
  * **Passage de Pré-commande Client (`BoutiqueDetailClient.tsx` & `DrawerCart.tsx`)** : Les clients scannant le QR code ou accédant au lien peuvent composer un panier multi-produits et transmettre la pré-commande en ligne + notification WhatsApp au marchand.
  * **Validation Instantanée & Conversion Marchand (`Commandes.tsx`)** : Panneau d'actions rapides sur chaque commande avec validation en 1 clic (`✅ Valider`), transformation automatique en **Facture PDF** (`📄 Générer Facture`) ou créance dans le carnet, et réponse instantanée pré-rédigée sur WhatsApp (`📲 Confirmer sur WhatsApp`).
- **Système de Recherche et Sélection Rapide de Produits et Clients dans les Documents et Carnets (16 août 2026)** 🔍 :
  ...
  * **Intégration dans le Carnet de Dettes & Créances (`CarnetDettes.tsx`)** : Ajout d'un champ de recherche `🔍 Rechercher un produit...` et de filtres par pilules de catégorie. Ajout des boutons d'action `-` (diminuer/supprimer) et `+` (augmenter) directement sur chaque carte de produit sélectionné, ainsi qu'un encadré récapitulatif avec bouton `🗑️ Vider tout le panier` et suppression unitaire `✕`.
