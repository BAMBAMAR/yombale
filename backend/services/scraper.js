...
async function sauvegarderProduits(items, marchandNom, siteUrl) {
  ...
  for(const item of items){
    try{
      ...
      // 2. Correspondance par similarité sur le nom normalisé (même catégorie uniquement)
          ...
          const {rows:fuzzy}=await pool.query(
            `SELECT id, nom,
             ...
             WHERE (LOWER(nom) LIKE '%' || $2 || '%'
               ...
               AND (categorie_id = $4 OR categorie_id IS NULL OR $4 IS NULL)
             ORDER BY sim DESC LIMIT 3`,
            [nomNorm, motsCles[0].toLowerCase(), '%' + motsCles.slice(0,2).join('%').toLowerCase() + '%', catId]
          );
