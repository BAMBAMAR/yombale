...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT o.*, m.nom AS marchand, m.logo_url
      FROM offres o JOIN marchands m ON m.id = o.marchand_id
      WHERE ($1::uuid IS NULL OR o.produit_id  = $1)
        AND ($2::uuid IS NULL OR o.marchand_id = $2)
...
// POST /api/offres/sync — reçoit les données d'un scraper
router.post('/sync', async (req, res) => {
  ...
  for (const p of produits) {
    ...
    await pool.query(
      'INSERT INTO historique_prix (offre_id, prix) SELECT id,$1 FROM offres WHERE produit_id=$2 AND marchand_id=$3',
