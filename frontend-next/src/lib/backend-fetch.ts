...
export async function backendFetch(
  ...
  try {
    return await fetch(primaryUrl, { ...options, headers, signal: options.signal ?? AbortSignal.timeout(15000) })
  } catch {
    ...
    return await fetch(fallbackUrl, { ...options, headers, signal: options.signal ?? AbortSignal.timeout(15000) })
  }
}
