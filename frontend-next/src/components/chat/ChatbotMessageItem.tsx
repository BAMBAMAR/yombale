...
function formatFcfa(val: number | null | undefined): string {
  if (!val) return 'Prix sur demande'
            ...
            {msg.items.slice(0, 3).map((item) => (
              ...
              >
                ...
                <div className="npl-chat-card-info">
                  <div className="npl-chat-card-title">{item.titre}</div>
                  <div className="npl-chat-card-price">{formatFcfa(item.prix)}</div>
                  {item.boutiqueNom && (
                    <div className="npl-chat-card-store">{item.boutiqueNom}</div>
                  )}
                </div>
            ...
            {msg.chips.map((chip, idx) => (
              chip.url.startsWith('/') ? (
                ...
                <button
                  key={idx}
                  type="button"
                  onClick={() => onChipClick && onChipClick(chip.label)}
                  className="npl-chat-chip-btn"
                >
                  {chip.label}
                </button>
