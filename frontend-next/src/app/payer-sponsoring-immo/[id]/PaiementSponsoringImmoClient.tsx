...
export default function PaiementSponsoringImmoClient({ immoId, titreCourt, settings, userId }: Props) {
  ...
  return (
    <div className="paiement-card">
      <div className="paiement-annonce-info">
        <span className="paiement-label">Annonce à sponsoriser</span>
      ...
      <div className="paiement-montant-box">
        ...
        <span className="paiement-montant-desc">Sponsoring Immo (30 jours)</span>
      </div>
      ...
      {showManuel && (
        <ModalPaiementManuel
          ...
          onSuccess={() => window.location.assign(`/immo/${immoId}`)}
        />
      )}
    </div>
  )
