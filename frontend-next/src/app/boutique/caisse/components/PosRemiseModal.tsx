    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', background: '#f1f5f9', padding: 4, borderRadius: 10, gap: 4 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              background: mode === 'pourcentage' ? '#ffffff' : 'transparent',
              color: mode === 'pourcentage' ? '#C75B00' : '#64748b',
              fontWeight: 800,
              fontSize: 13,
              cursor: 'pointer',
              boxShadow: mode === 'pourcentage' ? '0 2px 4px rgba(0,0,0,0.06)' : 'none',
            }}
          >
            Pourcentage (%)
          </button>
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              background: mode === 'montant' ? '#ffffff' : 'transparent',
              color: mode === 'montant' ? '#C75B00' : '#64748b',
              fontWeight: 800,
              fontSize: 13,
              cursor: 'pointer',
              boxShadow: mode === 'montant' ? '0 2px 4px rgba(0,0,0,0.06)' : 'none',
            }}
          >
            Montant Fixe (FCFA)
          </button>
            ...
            {PRESETS_PCT.map((p) => (
              <button
                ...
                style={{
                  padding: '8px 0',
                  borderRadius: 8,
                  border: Number(valeur) === p ? '2px solid #C75B00' : '1px solid #cbd5e1',
                  background: Number(valeur) === p ? '#fff7ed' : '#ffffff',
                  color: Number(valeur) === p ? '#C75B00' : '#0f172a',
                  fontWeight: 900,
                  fontSize: 13,
                  cursor: 'pointer',
                }}
        ...
        <form onSubmit={handleApply} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#f8fafc', padding: 12, borderRadius: 10, border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 12, color: '#64748b', fontWeight: 600 }}>Déduction sur le panier :</span>
            <span style={{ fontSize: 15, fontWeight: 900, color: '#dc2626' }}>-{fcfa(montantCalcule)}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {remiseActuelle > 0 && (
              <button
                ...
                style={{ flex: 1, padding: '12px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: 10, fontWeight: 800, cursor: 'pointer' }}
              >
                Supprimer
              </button>
            )}
            ...
            <button
              type="submit"
              style={{ flex: 1.5, padding: '12px', background: 'linear-gradient(135deg, #C75B00 0%, #ea580c 100%)', color: '#ffffff', border: 'none', borderRadius: 10, fontWeight: 900, fontSize: 14, cursor: 'pointer', boxShadow: '0 4px 12px rgba(199,91,0,0.3)' }}
            >
              ✓ Appliquer
            </button>
          </div>
