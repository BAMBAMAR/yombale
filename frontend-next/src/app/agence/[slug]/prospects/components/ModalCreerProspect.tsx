...
interface ModalCreerProspectProps {
  slug: string
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export function ModalCreerProspect({
  slug,
  isOpen,
  onClose,
  onSuccess,
}: ModalCreerProspectProps) {
    ...
    try {
      ...
      const res = await fetch(`/api/crm-immo/agence/${slug}/contacts`, {
        ...
        body: JSON.stringify({
          ...
          type_bien_souhaite: form.type_bien_souhaite,
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Erreur lors de la création du prospect.')
      }
